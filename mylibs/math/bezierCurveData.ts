...
export function bezierCurveDataPointAtParameter(data: IBezierCurveData, parameter: number, leftBezierCurveData: IBezierCurveData, rightBezierCurveData: IBezierCurveData): ICoordinate {
    ...
    var points = [clone(data.endPoint1), clone(data.controlPoint1), clone(data.controlPoint2), clone(data.endPoint2)];
    var leftCurve = [];
    var rightCurve = [];
...
export function bezierCurveDataSubcurveWithRange(data: IBezierCurveData, range: IRange) {
    ...
    var upperCurve = emptyObject<IBezierCurveData>();
    ...
    var lowerCurve = emptyObject<IBezierCurveData>();
...
export function bezierCurveDataRegularFatLineBounds(data: IBezierCurveData, range: IRange): INormilizedLine2D {
    ...
    var line = math.normalizedLineMake(data.endPoint1, data.endPoint2);
    ...
    var controlPoint1Distance = math.normalizedLineDistanceFromPoint(line, data.controlPoint1);
    var controlPoint2Distance = math.normalizedLineDistanceFromPoint(line, data.controlPoint2);
...
export function bezierCurveDataPerpendicularFatLineBounds(data: IBezierCurveData, range: IRange) {
    ...
    var normal = geometry.lineNormal(data.endPoint1, data.endPoint2);
    var startPoint = geometry.lineMidpoint(data.endPoint1, data.endPoint2);
    ...
    var controlPoint1Distance = math.normalizedLineDistanceFromPoint(line, data.controlPoint1);
    var controlPoint2Distance = math.normalizedLineDistanceFromPoint(line, data.controlPoint2);
    var point1Distance = math.normalizedLineDistanceFromPoint(line, data.endPoint1);
    var point2Distance = math.normalizedLineDistanceFromPoint(line, data.endPoint2);
...
export function bezierCurveDataBezierClipWithBezierCurve(data: IBezierCurveData, curve: IBezierCurveData, originalCurve: IBezierCurveData, originalRange: IRange) {
    ...
    var fatLineBounds: IRange = { minimum: undefined, maximum: undefined };
    var intersects;
    var fatLine = bezierCurveDataRegularFatLineBounds(curve, fatLineBounds);
    var regularClippedRange = bezierCurveDataClipWithFatLine(data, fatLine, fatLineBounds);
    ...
    var perpendicularLineBounds: IRange = { minimum: undefined, maximum: undefined };
    var perpendicularLine = bezierCurveDataPerpendicularFatLineBounds(curve, perpendicularLineBounds);
    var perpendicularClippedRange = bezierCurveDataClipWithFatLine(data, perpendicularLine, perpendicularLineBounds);
...
export function bezierCurveDataBoundingRect(data: IBezierCurveData): IRectData {
    ...
    var left = Math.min(data.endPoint1.x, Math.min(data.controlPoint1.x, Math.min(data.controlPoint2.x, data.endPoint2.x)));
    var top = Math.min(data.endPoint1.y, Math.min(data.controlPoint1.y, Math.min(data.controlPoint2.y, data.endPoint2.y)));
    var right = Math.max(data.endPoint1.x, Math.max(data.controlPoint1.x, Math.max(data.controlPoint2.x, data.endPoint2.x)));
    var bottom = Math.max(data.endPoint1.y, Math.max(data.controlPoint1.y, Math.max(data.controlPoint2.y, data.endPoint2.y)));
...
export function bezierCurveDataBounds(data: IBezierCurveData) {
    ...
    if (data.isStraightLine) {
        ...
        for (var i = 0; i < xRootsCount; i++) {
            ...
            var location = bezierCurveDataPointAtParameter(data, t, null, null);
...
export function bezierCurveDataCheckLinesForOverlap(data: IBezierCurveData, usRange: IRange, themRange: IRange, originalUs: IBezierCurveData, originalThem: IBezierCurveData, us: IBezierCurveData, them: IBezierCurveData): IBezierCurveData3 {
    ...
    var r = bezierCurveDataClipLineOriginalCurve(data, originalUs, us, usRange, them);
    us = r.data;
    if (!r.intersects)
        return { intersects: intersects, us: us, them: them };
...
export function bezierCurveDataConvertSelfAndPoint(data: IBezierCurveData, point: ICoordinate, bezierPoints: ICoordinate[]): void {
    var selfPoints:ICoordinate[] = [data.endPoint1, data.controlPoint1, data.controlPoint2, data.endPoint2];

    // c[i] in the paper
    var distanceFromPoint:ICoordinate[] = [];
    for (var i = 0; i < 4; i++)
        distanceFromPoint[i] = geometry.subtractPoint(selfPoints[i], point);

    // d[i] in the paper
    var weightedDelta:ICoordinate[] = [];
    for (var i = 0; i < 3; i++)
...
export function bezierCurveDataClosestLocationToPoint(me: IBezierCurveData, point: ICoordinate): ILocation {
    ...
    var location: ILocation = { parameter: undefined, distance: undefined };
...
export function bezierCurveDataCheckCurvesForOverlapRange(me: IBezierCurveData, intersectRange: IReference<IIntersectionRange>, usRange: IReference<IRange>, themRange: IReference<IRange>, originalUs: IBezierCurve, originalThem: IBezierCurve, us: IBezierCurveData, them: IBezierCurveData) {
    ...
    var usSubcurveRange: IRange = { minimum: null, maximum: null };
    var usSubcurve = bezierCurveDataFindPossibleOverlap(me, originalUs.data, them, usSubcurveRange);

    var themSubcurveRange: IRange = { minimum: null, maximum: null };
...
export function bezierCurveDataCheckNoIntersectionsForOverlapRange(me: IBezierCurveData, intersectRange: IReference<IIntersectionRange>, usRange: IRange, themRange: IRange, originalUs: IBezierCurve, originalThem: IBezierCurve, us: IBezierCurveData, them: IBezierCurveData): void {
    var r: IBezierCurveData3 = { us: null, them: null, intersects: false };
...
export function bezierCurveDataCheckForStraightLineOverlap(me: IBezierCurveData, intersectRange: IReference<IIntersectionRange>, usRange: IRange, themRange: IRange, originalUs: IBezierCurve, originalThem: IBezierCurve, us: IBezierCurveData, them: IBezierCurveData, nonpointUs, nonpointThem) {
    var res: IBezierCurveData3 = { intersects: false, us: null, them: null };
...
export function bezierCurveDataIntersectionsWithBezierCurve(me: IBezierCurveData, curve: IBezierCurveData, usRange: IReference<IRange>, themRange: IReference<IRange>, originalUs: IBezierCurve, originalThem: IBezierCurve, intersectRange: IReference<IIntersectionRange>, depth, outputBlock, stop) {
    ...
    var nonpointUs = makeRef(us);
    var nonpointThem = makeRef(them);
    ...
    while (iterations < maxIterations && ((iterations == 0) || (!geometry.rangeHasConverged(usRange.value, places) || !geometry.rangeHasConverged(themRange.value, places)))) {
        ...
        if (!bezierCurveDataIsPoint(them))
            nonpointThem.value = them;
        var r = bezierCurveDataBezierClipWithBezierCurve(nonpointUs.value, nonpointThem.value, originalUsData, usRange.value);
        us = r.data;
        ...
        if (percentChangeInUs < minimumChangeNeeded && percentChangeInThem < minimumChangeNeeded) {
            ...
            if (geometry.rangeGetSize(usRange) > geometry.rangeGetSize(themRange)) {
                ...
                var usRange1 = {value:geometry.rangeMake(usRange.value.minimum, (usRange.value.minimum + usRange.value.maximum) / 2.0)};
                var us1 = bezierCurveDataSubcurveWithRange(originalUsData, usRange1.value);
                var themRangeCopy1 = clone(themRange); // make a local copy because it'll get modified when we recurse

                var usRange2 = {value:geometry.rangeMake((usRange.value.minimum + usRange.value.maximum) / 2.0, usRange.value.maximum)};
                var us2 = bezierCurveDataSubcurveWithRange(originalUsData, usRange2.value);
                var themRangeCopy2 = clone(themRange); // make a local copy because it'll get modified when we recurse

                var range1ConvergedAlready = geometry.rangeHasConverged(usRange1.value, places) && geometry.rangeHasConverged(themRange.value, places);
                var range2ConvergedAlready = geometry.rangeHasConverged(usRange2.value, places) && geometry.rangeHasConverged(themRange.value, places);
                ...
                if (!range1ConvergedAlready && !range2ConvergedAlready && depth < maxDepth) {
                    ...
                    var leftIntersectRangeRef:IReference<IIntersectionRange> = {value:null};
                    ...
                    if (stop.value) //TODO: ts .value check if needed
                        return;
                    var rightIntersectRangeRef:IReference<IIntersectionRange> = {value:null};
                ...
                var themRange1 = makeRef(geometry.rangeMake(themRange.value.minimum, (themRange.value.minimum + themRange.value.maximum) / 2.0));
                var them1 = bezierCurveDataSubcurveWithRange(originalThemData, themRange1.value);
                var usRangeCopy1 = makeRef(clone(usRange.value));  // make a local copy because it'll get modified when we recurse

                var themRange2 = makeRef(geometry.rangeMake((themRange.value.minimum + themRange.value.maximum) / 2.0, themRange.value.maximum));
                var them2 = bezierCurveDataSubcurveWithRange(originalThemData, themRange2.value);
                var usRangeCopy2 = makeRef(clone(usRange.value));  // make a local copy because it'll get modified when we recurse

                var range1ConvergedAlready = geometry.rangeHasConverged(themRange1.value, places) && geometry.rangeHasConverged(usRange.value, places);
                var range2ConvergedAlready = geometry.rangeHasConverged(themRange2.value, places) && geometry.rangeHasConverged(usRange.value, places);

                if (!range1ConvergedAlready && !range2ConvergedAlready && depth < maxDepth) {
                    // Compute the intersections between the two halves of them and us
                    var leftIntersectRangeRef:IReference<IIntersectionRange> = makeRef<IIntersectionRange>();
                    ...
                    var rightIntersectRangeRef:IReference<IIntersectionRange> = makeRef<IIntersectionRange>();
