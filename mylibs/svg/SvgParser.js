...
//        }
      ...
      /**
        * Used for caching SVG documents (loaded via `Canvas#loadSVGFromURL`)
        * @property
        * @namespace
        */
       var svgCache = {
       ...
       function loadSVGFromURL(url, callback, reviver) {
         ...
         function onComplete(r) {
           ...
           if (!xml.documentElement && window.ActiveXObject && r.responseText) {
           ...
           parseSVGDocument(xml.documentElement, function (results, options) {
             svgCache.set(url, {
               objects: util.array.invoke(results, 'toObject'),
               options: options
             });
    ...
    function loadSVGFromString(string, callback, r)
    {
        string = string.trim();
        var doc;
