...
function visitElements(children, options, matrix) {
    ...
    for (var i = 0; i < children.length; ++i) {
        ...
        if (parseMethod) {
            ...
            var parsedAttributes = parseAttributes(child, attrs);
 ...
 */
function parseTransformAttribute(...args) {
...
export function loadSVGFromString(string, r?): Promise<IGroupContainer> {
    string = string.trim();
